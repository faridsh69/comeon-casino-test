      ...
      module = {
            ...
            if (module.is.searchSelection()) {
              $module
                .on(
                  ...
                  selector.menu,
                  module.event.menu.activate
                )
                .on(
                  "mouseup" + eventNamespace,
                  selector.menu,
                  module.event.menu.deactivate
                )
                .on("click" + eventNamespace, selector.search, module.show)
                .on(
                  "focus" + eventNamespace,
                  selector.search,
                  module.event.searchFocus
                )
                .on(
                  "blur" + eventNamespace,
                  selector.search,
                  module.event.searchBlur
                )
                .on(
                  "click" + eventNamespace,
                  selector.text,
                  module.event.searchTextFocus
                );
            } else {
              ...
              if (pressedKey == keys.upArrow) {
                $nextItem =
                  hasSelectedItem && inVisibleMenu
                    ? $selectedItem
                        .prevAll(
                          selector.item + ":not(." + className.filtered + ")"
                        )
                        .eq(0)
                    : $item.eq(0);
        ...
        is: {
          ...
          allFiltered: function () {
            return (
              $item.filter("." + className.filtered).length === $item.length
            );
          },
            ...
            if (
              ...
              if (settings.transition == "auto") {
                settings.transition = module.is.upward()
                  ...
                  : "slide down";
                module.verbose(
                  "Automatically determining animation based on animation direction",
                  settings.transition
                );
              }
