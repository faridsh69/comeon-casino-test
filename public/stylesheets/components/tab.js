        ...
        determineTabs: function () {
          ...
          if (settings.context === "parent") {
            if ($module.closest(selector.ui).length > 0) {
              ...
              module.verbose(
                "Using closest UI element for determining parent",
                $reference
              );
            } else {
              $reference = $module;
            ...
            if (isTab) {
              ...
              if (isLastTab && remoteContent) {
                if (!shouldIgnoreLoad) {
                  ...
                  settings.onTabInit.call(
                    $tab,
                    currentPath,
                    parameterArray,
                    historyEvent
                  );
                  settings.onTabLoad.call(
                    $tab,
                    currentPath,
                    parameterArray,
                    historyEvent
              ...
              if ($anchor && $anchor.length > 0 && currentPath) {
                module.debug(
                  "No tab found, but deep anchor link present, opening parent tab"
                );
                module.activate.all(currentPath);
                if (!module.cache.read(currentPath)) {
                  module.cache.add(currentPath, true);
