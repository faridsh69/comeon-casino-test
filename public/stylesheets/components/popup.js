  ...
  $.fn.popup = function (parameters) {
    ...
    $allModules.each(function () {
      ...
      module = {
        ...
        hideAll: function () {
          $(selector.popup).filter(":visible").transition(settings.transition);
        },
        ...
        set: {
          position: function (position, arrowOffset) {
              ...
              marginLeft = settings.inline
                ? parseInt(
                    window
                      .getComputedStyle(targetElement)
                      .getPropertyValue(
                        module.is.rtl() ? "margin-right" : "margin-left"
                      ),
                    10
                  )
                : 0,
              target =
                settings.inline || settings.popup
                  ? $target.position()
                  : $target.offset(),
              computedPosition,
              positioning,
            ...
            if (searchDepth == settings.maxSearchDepth && settings.lastResort) {
              module.debug(
                "Using last resort position to display",
                settings.lastResort
              );
            ...
            if (module.is.rtl()) {
              computedPosition = computedPosition.replace(
                /left|right/g,
                function (match) {
