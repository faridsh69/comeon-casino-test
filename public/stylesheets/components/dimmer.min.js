      ...
      a.each(function () {
        ...
        (f = {
          preinitialize: function () {
            f.is.dimmer()
              ...
              : ((l = S),
                (u = f.has.dimmer()
                  ? g.dimmerName
                    ? l.children(h.dimmer).filter("." + g.dimmerName)
                    : l.children(h.dimmer)
                  : f.create()));
          },
          initialize: function () {
            f.debug("Initializing dimmer", g),
              "hover" == g.on
                ? l.on("mouseenter" + C, f.show).on("mouseleave" + C, f.hide)
                : "click" == g.on && l.on(D + C, f.toggle),
              f.is.page() &&
                (f.debug("Setting as a page dimmer", l), f.set.pageDimmer()),
              f.is.closable() &&
                (f.verbose("Adding dimmer close event", u),
                u.on(D + C, f.event.click)),
              f.set.dimmable(),
              f.instantiate();
          },
          instantiate: function () {
            f.verbose("Storing instance of module", f), (N = f), S.data(w, N);
          },
          destroy: function () {
          ...
          event: {
            click: function (i) {
              f.verbose("Determining if event occured on dimmer", i),
                (0 === u.find(i.target).length || e(i.target).is(h.content)) &&
          ...
          has: {
            dimmer: function () {
              return g.dimmerName
                ? S.children(h.dimmer).filter("." + g.dimmerName).length > 0
                : S.children(h.dimmer).length > 0;
            },
          },
          is: {
            active: function () {
            ...
            dimmer: function () {
              return S.is(h.dimmer);
            },
            dimmable: function () {
              return S.is(h.dimmable);
            },
            dimmed: function () {
          ...
          invoke: function (i, n, a) {
            ...
            return (
              ...
              e.isFunction(m) ? (s = m.apply(a, n)) : m !== t && (s = m),
              e.isArray(o)
                ? o.push(s)
                : o !== t
                ? (o = [o, s])
          ...
          f.preinitialize(),
          d
            ? (N === t && f.initialize(), f.invoke(s))
            : (N !== t && f.destroy(), f.initialize());
      }),
      o !== t ? o : this
    );
  }),
