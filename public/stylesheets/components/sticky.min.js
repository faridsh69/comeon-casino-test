    ...
    return (
      s.each(function () {
        var s,
          u,
          d,
          h = e.extend(!0, {}, e.fn.sticky.settings, o),
          p = h.className,
          g = h.namespace,
          b = h.error,
          v = "." + g,
        ...
        (d = {
          initialize: function () {
            return (
              (s = h.context ? e(h.context) : y),
              0 === s.length
                ? void d.error(b.invalidContext, h.context, C)
                : (d.verbose("Initializing sticky", h, y),
                  d.save.positions(),
                  d.is.hidden() && d.error(b.visible, C),
                  d.cache.element.height > d.cache.context.height
                    ? (d.reset(), void d.error(b.elementSize, C))
                    : (S.on("resize" + v, d.event.resize),
                      k.on("scroll" + v, d.event.scroll),
                      d.observeChanges(),
                      void d.instantiate()))
            );
          },
          ...
          destroy: function () {
            d.verbose("Destroying previous module"),
              d.reset(),
              u && u.disconnect(),
              S.off("resize" + v, d.event.resize),
              k.off("scroll" + v, d.event.scroll),
              C.removeData(x);
          },
              ...
              ((u = new MutationObserver(function () {
                clearTimeout(d.timer),
                  (d.timer = setTimeout(function () {
                    d.verbose("DOM tree modified, updating sticky menu"),
                      d.refresh();
                  }, 200));
              })),
              u.observe(z, { childList: !0, subtree: !0 }),
          ...
          get: {
            ...
            scrollChange: function (e) {
              return (
                (e = e || k.scrollTop()), d.lastScroll ? e - d.lastScroll : 0
              );
            },
          ...
          set: {
            offset: function () {
              d.verbose("Setting offset on element", h.offset),
                C.css("margin-top", h.offset);
            },
            containerSize: function () {
              var e = y.get(0).tagName;
              "HTML" === e || "body" == e
                ? (y = C.offsetParent())
                : (d.debug("Settings container size", d.cache.context.height),
                  y.height(d.cache.context.height));
            },
            scroll: function (e) {
          ...
          is: {
            top: function () {
              return C.hasClass(p.top);
            },
            bottom: function () {
              return C.hasClass(p.bottom);
            },
            ...
            bound: function () {
              return C.hasClass(p.bound);
            },
            fixed: function () {
              return C.hasClass(p.fixed);
            },
          },
          stick: function () {
            var e = d.cache,
              ...
              n = e.window,
              i = e.context,
              s = d.is.bottom() && h.pushing ? h.bottomOffset : h.offset,
              r = {
                top: k.scrollTop() + s,
            ...
            d.save.scroll(r.top),
              l &&
                (d.is.initialPosition()
                  ? r.top >= o.top &&
                    (d.debug("Element passed, fixing element to page"),
                    d.fixTop())
                  : d.is.fixed()
                  ? d.is.top()
                    ? r.top < o.top
          ...
          fixTop: function () {
            d.debug("Fixing element to top of page"),
              d.set.offset(),
              C.css("left", d.cache.element.left)
                .removeClass(p.bound)
                .removeClass(p.bottom)
                .addClass(p.fixed)
                .addClass(p.top),
          ...
          unbind: function () {
            d.debug("Removing absolute position on element"),
              d.remove.offset(),
              C.removeClass(p.bound).removeClass(p.top).removeClass(p.bottom);
          },
          unfix: function () {
            d.debug("Removing fixed position on element"),
              d.remove.offset(),
              C.removeClass(p.fixed).removeClass(p.top).removeClass(p.bottom),
              h.onUnstick.call(z);
          },
          reset: function () {
            d.debug("Reseting elements position"),
              d.unbind(),
              d.unfix(),
              d.resetCSS();
          },
          resetCSS: function () {
            C.css({ top: "", bottom: "", width: "", height: "" }),
              y.css({ height: "" });
          },
          ...
          invoke: function (t, o, s) {
            ...
            return (
              ...
              e.isFunction(c) ? (a = c.apply(s, o)) : c !== n && (a = c),
              e.isArray(i)
                ? i.push(a)
                : i !== n
                ? (i = [i, a])
          ...
          f
            ? (T === n && d.initialize(), d.invoke(l))
            : (T !== n && d.destroy(), d.initialize());
      }),
      i !== n ? i : this
    );
  }),
